...
class MarketplaceWalkthroughController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Walkthrough::with('attachment')->where('type','marketplace');
        ...
        return view('user::marketplace_walkthrough.index',compact('walkthroughs','roles'));
    }
    ...
    {
        $roles = Role::select('role_id','name')->whereNotIn('slug',['super_admin','admin','importer','distributer'])->orderBy('order')->get();
        return view('user::marketplace_walkthrough.create',compact('roles'));
    }
    ...
    public function store(Request $request)
    {
        ...
        {   
            ...
            $walkthrough->type = 'marketplace';
            $walkthrough->save();

            if(array_key_exists('point_title_en',$input)){

                foreach($input['point_title_en'] as $key => $value){
                    if($value){
                        $data = [];
                        $data['title_en'] = $value;
                        $data['title_it'] = $input['point_title_it'][$key];
                        $data['description_en'] = $input['point_description_en'][$key];
                        $data['description_it'] = $input['point_description_it'][$key];
                        $data['icon_id'] = $this->uploadWalkthroughImage($input['icons'][$key]);
                        $data['walk_through_screen_id'] = $walkthrough->walk_through_screen_id;
                        WalkThroughPoint::create($data);
                    }
                }
            }
    ...
    public function edit($id)
    {
        ...
        if($walkthrough){
            $points = WalkThroughPoint::with('attachment')->where('walk_through_screen_id',$id)->get();
        }
        return view('user::marketplace_walkthrough.edit',compact('walkthrough','roles','points'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        {   
            ...
            if(array_key_exists('point_title_en',$input)){
                WalkThroughPoint::where('walk_through_screen_id',$id)->delete();

                foreach($input['point_title_en'] as $key => $value){
                    if($value){

                        if(array_key_exists('icons',$input) && array_key_exists($key,$input['icons'])){
                            $icon_id = $this->uploadWalkthroughImage($input['icons'][$key]);
                        }else{
                            $icon_id = $input['icon_ids'][$key];
                        }
                        ...
                        $data['walk_through_screen_id'] = $id;
                        WalkThroughPoint::create($data);
                    }
                }
            }
